...
const compareIterablesDeep = (
  ...
  let nextB = iteratorB.next();
  while (!nextA.done && !nextB.done) {
    if (!deep(nextA.value, nextB.value)) {
      return false;
    }
    nextA = iteratorA.next();
    nextB = iteratorB.next();
...
export function deep<T>(valueA: T, valueB: T): boolean {
  if (Object.is(valueA, valueB)) {
    return true;
  }
  if (
