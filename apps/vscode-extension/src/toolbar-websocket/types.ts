import type { LogEntry } from './handlers/console-logs';

// --- Define specific Payloads for Server -> Client Requests ---

// Example: getURL
type GetUrlRequestPayload = Record<string, never>;
type GetUrlResponsePayload = {
  url: string;
};

// Example: getElementText
interface GetElementTextRequestPayload {
  selector: string;
}
interface GetElementTextResponsePayload {
  text: string | null;
}

// Example: Prompt result
interface ProcessPromptResponsePayload {
  success: boolean;
  result?: any;
}

// Example: Log batch acknowledgement
interface LogBatchAckPayload {
  success: boolean;
...
export interface ServerRequestMap {
  ...
  getElementText: {
    request: GetElementTextRequestPayload;
    response: GetElementTextResponsePayload;
  };
  processPrompt: {
    request: { prompt: string };
    response: ProcessPromptResponsePayload;
  };
...
interface ClientPromptPayload {
  prompt: string;
}
interface ClientLogBatchPayload {
  logs: LogEntry[];
}
