...
export async function callCursorAgent(prompt: string): Promise<void> {
    ...
    try {
        ...
        console.error(">>> Using range:", `Start=[L${selectionOrFullDocRange.start.line},C${selectionOrFullDocRange.start.character}], End=[L${selectionOrFullDocRange.end.line},C${selectionOrFullDocRange.end.character}]`);
        ...
        if (document) { // Check if document still valid (it should be)
            console.error(">>> Clearing fake diagnostic from:", document.uri.toString());
            ...
            console.error(">>> Document not available in finally block, clearing all diagnostics from collection.");
