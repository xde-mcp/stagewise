...
export function shouldChromeConsumeEvent(e: KeyboardEvent): boolean {
  ...
  if (isEditable) {
    // [Insert previous isNativeInputEvent logic here]
    // Recapping the simplified version:
    // If it's a modifier shortcut (Cmd+S), let it tunnel (return false).
    // If it's navigation/typing, keep it (return true).

    // Exception: Input types that aren't really text
    ...
    if (
      ...
      // If Cmd/Ctrl is held, usually tunnel, unless it's standard text nav (Cmd+A/C/V)
      // If Cmd/Ctrl is held, usually tunnel, unless it's standard text nav
      if (e.metaKey || e.ctrlKey) {
        ...
        // Otherwise, it's likely a browser shortcut (Cmd+R, Cmd+P, etc) -> Tunnel it
        return false;
      }
      return true; // Simple typing/arrows in text box -> consume
    }
  }
