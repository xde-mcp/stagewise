...
export const Omnibox = ({
  ...
  const omniboxSuggestions = useOmniboxSuggestions(
    displayedTabUrl,
    inputValue,
    searchEngines,
    defaultEngineId,
    ...
    <form onSubmit={onSubmit} className="flex-1">
      ...
      >
        ...
        <Autocomplete.Portal>
          ...
          >
            <Autocomplete.Popup
              className="data-ending-style:-translate-y-2 data-starting-style:-translate-y-2 max-h-72 w-[calc(var(--anchor-width)-0.5rem)] max-w-[calc(var(--anchor-width)-0.5rem)] rounded-xl bg-surface-1 p-1 text-foreground shadow-lg ring-1 ring-derived-subtle transition-all duration-150 ease-out data-ending-style:scale-y-95 data-starting-style:scale-y-95 data-ending-style:rounded-t-none data-startinstyle:rounded-t-none data-ending-style:opacity-0 data-starting-style:opacity-0"
              finalFocus={false}
            >
              {omniboxSuggestions.length === 0 && (
              ...
              <Autocomplete.List className="divide-y divide-surface-2">
                {omniboxSuggestions.map((group, gIdx) => (
                  <Autocomplete.Group
