...
- You can publish your plugin to npm or use it locally

## 5.1. Using Local NPM Linking with pnpm

During development, you may want to test your plugin in a real Stagewise-enabled app before publishing it to npm. You can do this using local linking with `pnpm`.

### Why use local linking?
Local linking lets you make changes to your plugin and see them reflected immediately in your example or test project, without publishing to the npm registry.
...
> **Tip:** When you're done, you can unlink with `pnpm unlink --global @stagewise/plugin-example` in your example app and `pnpm unlink --global` in your plugin directory.
